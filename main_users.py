...
api.add_resource(JobsListResource, '/api/v2/jobs')  # Для работы со списком работ
api.add_resource(JobsResource, '/api/v2/jobs/<int:job_id>')  # Для работы с конкретной работой
...
def addjob():
    ...
    if add_form.validate_on_submit():
        ...
        jobs = Jobs(
            ...
            user_id=1  # Временно! Нужно брать из сессии залогиненного пользователя
        )
        db_sess.add(jobs)
        db_sess.commit()
        return redirect('/')
...
def main():
    ...
    # app.register_blueprint(jobs_api.blueprint) # Удаляем старую регистрацию
    app.run()
